...
func TestParseCommand(t *testing.T) {
	...
	assert.Equal(t, "FROM:<test@test.te> 8BITMIME", cmd.data, "parsing 'MAIL FROM:<test@test.te> 8BITMIME' should parse 'FROM:<test@test.te> 8BITMIME' as data")
	...
	assert.Equal(t, "", cmd.data, "parsing 'RSET' command with no arguments should parse empty data")
